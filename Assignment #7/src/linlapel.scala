...
case class Equiv(eq: Map[LogicV, Value]) {
  ...
  def apply(v: Value): Value = v match {
    case logicV: LogicV => if (eq.get(logicV).isDefined) apply(eq(logicV)) else logicV
    case FunV(name, args) => FunV(name, args.map(apply))
    case _ => v
  } // SAME AS ASSIGN6

  // unify two values
  def unify(v1: Value, v2: Value): Option[Equiv] = (apply(v1), apply(v2)) match {
    case (funV_1@FunV(name_1, args_1), funV_2@FunV(name_2, args_2)) =>
      if (name_1 != name_2 || args_1.length != args_2.length) None
      else args_1.zip(args_2).foldLeft(Some(Equiv(eq)): Option[Equiv])((option, tuple) => option match {
        case Some(equiv) => equiv.unify(tuple._1, tuple._2)
        case _ => None
      })
    case (logicV_1@LogicV(id_1), logicV_2@LogicV(id_2)) => if (logicV_1 == logicV_2 || id_1 == id_2) Some(Equiv(eq)) else Some(Equiv(eq + (logicV_1 -> logicV_2)))
    case (logicV@LogicV(id), funV@FunV(name, arg)) => if (funV.contains(logicV)) None else Some(Equiv(eq + (logicV -> funV)))
    case (funV@FunV(name, arg), logicV@LogicV(id)) => if (funV.contains(logicV)) None else Some(Equiv(eq + (logicV -> funV)))
    case _ => None
  } // SAME AS ASSIGN6
...
object Interpreter {
  ...
  def eval(goals: Seq[TODO], _Δ: Seq[Assume], eq: Equiv): Unit =
    if (goals nonEmpty) goals.head match {
      ...
      case goalClo@Closure(_: Predicate, _, _Γ) ⇒
        val tail: Seq[TODO] = goals.tail

        def factCase(vars: Set[Var], pred: Predicate, as: Seq[Assume]): Unit = {
          val varToLogicV: Env = vars.map(_ -> LogicV()).toMap
          val closure: Closure = Closure(pred, varToLogicV, _Γ)
          matchPredicates(closure, goalClo, eq) match {
            case Some(equiv: Equiv) => eval(tail, as, equiv)
            case _ => Unit
          }
        }

        def clauseCase(vars: Set[Var], hd: Predicate, body: Goal, as: Seq[Assume]): Unit = {
          val varToLogicV: Map[Var, LogicV] = vars.map(_ -> LogicV()).toMap
          val closure_1: Closure = Closure(hd, varToLogicV, _Γ)
          matchPredicates(closure_1, goalClo, eq) match {
            case Some(equiv: Equiv) =>
              val closure_2: Closure = Closure(body, varToLogicV, _Γ)
              eval(closure_2 +: tail, as, equiv)
            case _ => Unit
          }
        }

        _Δ.foldLeft(0)((i: Int, assume: Assume) => {
          val assumes: Seq[Assume] = _Δ.splitAt(i)._1 ++ _Δ.splitAt(i)._2.tail
          assume match {
            case value@With(as: Seq[Assume]) => as.map({
              case fact@Fact(vars: Set[Var], pred: Predicate) =>
                factCase(vars, pred, assumes)
              case clause@Clause(vars: Set[Var], hd: Predicate, body: Goal) =>
                clauseCase(vars, hd, body, assumes)
              case _ => Unit
            })
            case fact@Fact(vars: Set[Var], pred: Predicate) =>
              factCase(vars, pred, assumes)
        ...
        _Γ.map(_ match {
          case fact@Fact(vars: Set[Var], pred: Predicate) =>
            factCase(vars, pred, _Δ);
          case clause@Clause(vars: Set[Var], hd: Predicate, body: Goal) =>
            clauseCase(vars, hd, body, _Δ)
          case value@With(as) => as.map({
            case fact@Fact(vars: Set[Var], pred: Predicate) =>
              factCase(vars, pred, _Δ);
            case clause@Clause(vars, hd, body) =>
              clauseCase(vars, hd, body, _Δ.diff(Seq(value)))
            case _ => Unit
          })
          case _ => Unit
        }) // FILL ME IN
      ...
      case Closure(MulConjunct(g1, g2), env, _Γ) ⇒
        val closure_1: Closure = Closure(g1, env, _Γ)
        val closure_2: Closure = Closure(g2, env, _Γ)
        eval(closure_1 +: closure_2 +: goals.tail, _Δ, eq) // FILL ME IN
      ...
      case Closure(AddConjunct(g1, g2), env, _Γ) ⇒
        ...
        val closure_2: Closure = Closure(g2, env, _Γ)
        val tail: Seq[TODO] = goals.tail
        eval(closure_1 +: AC_Step1(_Δ) +: closure_2 +: tail, _Δ, eq) // FILL ME IN
      ...
      case Closure(Disjunct(g1, g2), env, _Γ) ⇒
        ...
        val closure_2: Closure = Closure(g2, env, _Γ)
        val tail: Seq[TODO] = goals.tail
        eval(closure_1 +: tail, _Δ, eq)
        eval(closure_2 +: tail, _Δ, eq) // FILL ME IN
      ...
      case Closure(LinHypothetical(a, g), env, _Γ) ⇒
        val closure: Closure = Closure(g, env, _Γ)
        val step: LH_Step = LH_Step(a, _Δ.count(a == _))
        val tail: Seq[TODO] = goals.tail
        eval(closure +: step +: tail, a +: _Δ, eq) // FILL ME IN

      // classical hypothetical implication goal
      case Closure(Hypothetical(a, g), env, _Γ) ⇒
        val closure: Closure = Closure(g, env, a +: _Γ)
        eval(closure +: goals.tail, _Δ, eq) // FILL ME IN

      // escape goal; we insert ESC_Step after proving g to handle
      // linear assumption passing style.
      case Closure(Escape(g), env, _Γ) ⇒
        val closure: Closure = Closure(g, env, _Γ)
        val step: ESC_Step = ESC_Step(_Δ)
        val tail: Seq[TODO] = goals.tail
        eval(closure +: step +: tail, Seq[Assume](), eq) // FILL ME IN

      // existential quantification goal
      case Closure(Exists(vars, g), env, _Γ) ⇒
        val left: Env = vars.foldLeft(env)((env: Env, value: Var) => env + (value -> LogicV()))
        val closure: Closure = Closure(g, left, _Γ)
        val tail: Seq[TODO] = goals.tail
        eval(closure +: tail, _Δ, eq) // FILL ME IN
      ...
      case AC_Step1(_Δ1) ⇒
        val tail: Seq[TODO] = goals.tail
        tail.head match {
          case closure@Closure(goal: Goal, env: Env, as: Seq[Assume]) =>
            val step: AC_Step2 = AC_Step2(_Δ)
            eval(closure +: step +: tail.tail, _Δ1, eq)
          case _ => Unit
        } // FILL ME IN
      ...
      case AC_Step2(_Δ1) ⇒
        if (_Δ == _Δ1) eval(goals.tail, _Δ, eq) // FILL ME IN
      ...
      case LH_Step(a, num) ⇒
        if (_Δ.count(a.equals(_)) <= num) eval(goals.tail, _Δ, eq) // FILL ME IN
    }
  ...
  def matchPredicates(aClo: Closure, gClo: Closure, eq: Equiv): Option[Equiv] = (aClo.g, gClo.g) match {
    case (Predicate(name_1, args_1), Predicate(name_2, args_2)) =>
      if (name_1 != name_2 || args_1.size != args_2.size) None
      else {
        val map_1: Seq[Value] = args_1.map(termToValue(_, aClo.env))
        val map_2: Seq[Value] = args_2.map(termToValue(_, gClo.env))
        val zippedTuples: Seq[(Value, Value)] = map_1.zip(map_2)
        zippedTuples.foldLeft[Option[Equiv]](Some(eq))((option, tuple) => option match {
          case Some(equiv) => equiv.unify(tuple._1, tuple._2)
          case _ => None
        })
      }
