...
toisaroot(Component, [Component|Other]) :- not(Component = 'cellular component'), is_a(Component, Part), toisaroot(Part, Other).

subconcept(C1, C2) :- part_of(C1, C2); is_a(C1, C2).
subconcept(C1, C2) :- (is_a(C3, C2); part_of(C3, C2)), subconcept(C1, C3).