...
object Images {
  ...
  // !!FILL ME IN
   def lift0[T](t : T) : Img[T] = (point : Point) => t
  ...
  // !!FILL ME IN
  def lift1[A, B](func: A => B)(img: Img[A]): Img[B] = (point : Point) => func(img(point))
  ...
  // of type C.
  // !!FILL ME IN
  def lift2[A, B, C](func: (A , B) => C)(image_A: Img[A])(image_B: Img[B]): Img[C] = (point : Point) => func(image_A(point), image_B(point))
  ...
  // !!FILL ME IN
  def lift3[A, B, C, D](func : (A, B, C) => D)(image_A: Img[A])(image_B: Img[B])(image_C: Img[C]): Img[D] = (point:Point) => func(image_A(point),image_B(point),image_C(point))
  ...
  // ImgMask argument.
  // !!FILL ME IN
  val selectImgClr = lift3((bool: Boolean, color1: Colour, color2: Colour) => if(bool) color1 else color2) _
  ...
  // !!FILL ME IN
  val msk2clr = lift1((bool: Boolean)=> if(bool) ExampleClr.black else ExampleClr.white ) _
...
object SpatialTransforms {
  ...
  // y coordinate.
  // !!FILL ME IN
  def translatePt(vector: Vector): TransformPt = (point : Point) => Point(point.x + vector.x, point.y + vector.y)
  ...
  // coordinate.
  // !!FILL ME IN
  def scalePt(vector: Vector): TransformPt = (point : Point) => Point(point.x * vector.x, point.y * vector.y)
  ...
  // !!FILL ME IN
  def rotatePt(theta : Double): TransformPt = (point : Point) => Point(point.x * cos(theta) - point.y * sin(theta), point.y * cos(theta) + point.x * sin(theta))
  ...
  // !!FILL ME IN
  def swirlPt(value: Double): TransformPt = (point: Point) => { 
    val theta = Utility.dist(point) * ( (2 * PI) / value)
    Point(point.x * cos(theta) - point.y * sin(theta), point.y * cos(theta) + point.x * sin(theta))
 }

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TransformPt.
  // !!FILL ME IN
  def polarXf(xform: TransformPlr): TransformPt = (point: Point) => plr2pt(xform(pt2plr(point)))
  ...
  // !!FILL ME IN
  def translate[A](vector: Vector): (Point => A) => (Point => A) = (func: Point => A) => (point:Point) => func(translatePt(vector.neg)(point))
  ...
  // !!FILL ME IN
  def scale[A](vector: Vector): (Point => A) => (Point => A) = (func: Point => A) => (point:Point) => func(scalePt(vector.inverse)(point))
  ...
  // !!FILL ME IN
  def rotate[A](theta: Double): (Point => A) => (Point => A) = (func: Point => A) => (point:Point) => func(rotatePt(-theta)(point)) 
  ...
  // !!FILL ME IN
  def swirl[A](theta: Double): (Point => A) => (Point => A) = (func: Point => A) => (point:Point) => func(swirlPt(-theta)(point))
}
...
object Masking {
  ...
  // !!FILL ME IN
  val notM = lift1((bool: Boolean) => !bool) _
  ...
  // !!FILL ME IN
  val intersectM = lift2((bool1: Boolean, bool2: Boolean) => bool1 && bool2) _
  ...
  // !!FILL ME IN
  val unionM = lift2((bool1: Boolean, bool2: Boolean) => bool1 || bool2) _ 
  ...
  // !!FILL ME IN
  val xorM = lift2((bool1: Boolean, bool2: Boolean) => (bool1 || bool2) && !(bool1 && bool2)) _
