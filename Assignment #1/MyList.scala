...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  def flatMap[B](f: A => MyList[B]): MyList[B] = f(x).append(xs.flatMap[B](f))
  ...
  def append(other: MyList[A]): MyList[A] = MyCons[A](x, xs.append(other))

  def foldLeft[B](initial: B)(f: (B, A) => B): B = {
    def fold(xs: MyList[A], accum: B): B =
      if (xs.isEmpty) accum 
      else fold(xs.tail, f(accum, xs.head))
    fold(MyCons[A](x,xs), initial)
  }

  def foldRight[B](initial: B)(f: (A, B) => B): B = {
    def fold(list: MyList[A], accum: B): B = {
      if (list.tail.isEmpty) f(list.head, accum) 
      else f(list.head, fold(list.tail, accum))
    }
    fold(MyCons[A](x, xs), initial)
  }
  ...
  def init: MyList[A] = {
    if (xs.isEmpty) MyNil[A] 
    else MyCons[A](x, xs.init)
  }

  def last: A = {
    def last(item: A, list: MyList[A]) : A = {
      if(list.isEmpty) item 
      else last(list.head, list.tail)
    }
    last(x, xs)
  }
